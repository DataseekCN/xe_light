package com.dataseek.xe.util;

public class XeConsts {
    //etsy app开发者账户名
    public static final String APP_ETSY_DEVELOPER_ACCOUNT ="etsy@dataseek.info";
    //xero app开发者账户名
    public static final String APP_XERO_DEVELOPER_ACCOUNT ="xero@dataseek.info";
    //请求业务状态-success
    public static final String RESPONSE_STATUS_SUCCESS="success";
    //请求业务状态-failure
    public static final String RESPONSE_STATUS_FAILURE="failure";
    //oauth参数名:oauth_token
    public static final String OAUTH_TOKEN_NAME="oauth_token";
    //oauth参数名:oauth_token_secret
    public static final String OAUTH_TOKEN_SECRET_NAME="oauth_token_secret";
    //oauth参数名：oauth_callback
    public static final String OAUTH_CALLBACK_NAME="oauth_callback";
    //申请流程状态:等待授权
    public static final String AUTH_STATUS_WAIT_AUTHORIZE="wait_authorize";
    //申请流程状态:已授权
    public static final String AUTH_STATUS_AUTHORIZED="authorized";

}
